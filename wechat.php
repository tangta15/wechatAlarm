...
class HardWare{
    ...
    private static function auto_return_message(){
        ...
        if (strtolower($postObj->MsgType)== 'text') {   	
    		...
    		$info= sprintf($template,$to_user,$from_user,$time,$msg_type,$content);
    		echo $info;
    		file_put_contents('test.txt', "xcxz");
    	}
	}  
	...
	private static function response_device_status(){
    	$postArr= $GLOBALS['HTTP_RAW_POST_DATA'];
    	$postObj= json_decode($postArr,true);
    	file_put_contents('test.txt', $postObj);
    	if (strtolower($postObj->msg_type) == "notify"){
    		$ch = curl_init(); //初始化一个CURL对象
			$token= new GetToken();
			$access_token= $token->read_token();
    	...
    	if (strtolower($postObj->msg_type) == "notify"){
    		$ch2 = curl_init(); //初始化一个CURL对象
			$url2= "https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token=".$access_token;
	        curl_setopt($ch2, CURLOPT_URL,$url2);//设置你所需要抓取的URL
	        curl_setopt($ch2, CURLOPT_SSL_VERIFYPEER, false);
	        curl_setopt($ch2, CURLOPT_SSL_VERIFYHOST, false);
	        curl_setopt($ch2, CURLOPT_RETURNTRANSFER, 1);//设置curl参数，要求结果是否输出到屏幕上，为true的时候是不返回到网页中,假设上面的0换成1的话，那么接下来的$data就需要echo一下。
	        curl_setopt($ch2, CURLOPT_POST, 1);//设置curl参数，要求以post方式.
	        $post_data2 = '{
				    "touser":"oUybXwUiFdVqR2sQhIXSlJ2B4Fj8",
				    "msgtype":"text",
				    "text":
				    {
				         "content":"Hello World"
